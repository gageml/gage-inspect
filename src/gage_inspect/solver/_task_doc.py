...
def task_doc(
    ...
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        # Solver only works with input strings
        if isinstance(state.input, list):
            raise ValueError(
                f"task_doc solver requires string input but got {state.input!r}"
            )
        ...
        prompt = [
            resolve(
                prologue or PROLOGUE_TEMPLATE, task_description=doc.description.strip()
            )
        ]
        ...
        if doc.params:
            ...
            # Multiple params require parseable YAML/JSON for values
                ...
                try:
                    data = yaml.safe_load(state.input_text)
            ...
            prompt.append(INPUTS_TEMPLATE.format(inputs=state.input_text))
        ...
        # Set user prompt and generate response
        state.user_prompt.content = "\n\n".join(prompt)
